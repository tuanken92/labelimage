...
def ShowGroundTruth(img_path1, anno_path,im):
	...
	for obj in root.findall('object'):			
		...
		xmin = int(bndbox.find('xmin').text) -300
		ymin = int(bndbox.find('ymin').text) -480
		xmax = int(bndbox.find('xmax').text) -300
		ymax = int(bndbox.find('ymax').text)-480
	...
	if xmax is None:
		os.remove(anno_path)
		os.remove(img_path1)
	if (xmax < 1350) and (ymax < 450) and (xmin >0 ) and (ymin > 0):
		tree.write(anno_path)
		cv2.imwrite(img_path1, im)
	else:
		os.remove(anno_path)
		os.remove(img_path1)
...
if __name__ == '__main__':    
	anno_dir="/home/vsmart/Works/labelImg/oto/image/Image_Oto_in_0507_8h_FHD/"	
	frame_dir="/home/vsmart/Works/labelImg/oto/image/Image_Oto_in_0507_8h_FHD/"
	...
	while i <np:		
		...
		#frame_crop = frame[480:480+450, 300:300+1350]	
